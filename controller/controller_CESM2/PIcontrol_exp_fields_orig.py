...
if not firsttime==1: # Once integrator has been activated, always leave it on / Can only be executed if log file already exists, hence 'not firsttime' is required
    ...
    if previous_int_weight==1:
        int_weight = 1
elif abs(de)<int_onset_threshold: # Activate integrator once temperature error is within acceptable range
    int_weight = 1
elif timestamp >= (firstyear + int_failsafe_time): # Activate integrator after defined time period has passed
    int_weight = 1
