...
def extractvars(varname,varloc,runname,whichnames,archivedir,scratchdir):
    print('DEBUG extractvars', varname)
    ...
    os.system('ncrcat '+scratchdir+'/'+varname+'*.nc '+scratchdir+'/'+varname+'_out.nc') # LvK
    #vals,lats,lons,times=readfile(scratchdir+'/'+varname+'_out.nc',varname) #LvK hack for SE

    with xr.open_dataset(os.path.join(scratchdir,varname+'_out.nc')) as ds: # LvK
            da_var = ds[varname].load() #LvK

    os.system('rm -r '+scratchdir)
    #return vals,lats,lons,times # LvK
    return da_var # LvK
...
for k in range(len(variables)):
    ...
    da_var = extractvars(variables[k],varlocs[k],runname,filestoextract[k],archivepaths[k],scratchdir) #LvK
    #print(filestoextract[k])
    #outvals.append(vals)

with xr.open_dataset(os.path.join(archivepaths[0], filestoextract[0][0])) as ds:
    da_area = ds['area'].load()
