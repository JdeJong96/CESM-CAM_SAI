...
def int_l2_lev(ds,name):
    ...
    integral = integrand.integrate(dim='lat').integrate(dim='lev')
...
def int_mass(ds,name):
    ...
    integral = (field * dps).integrate(dim='lev') # integrate in pressure space using dps are p intervals -> gives column burden at latitudes
